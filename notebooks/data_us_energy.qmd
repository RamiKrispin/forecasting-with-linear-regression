...
```{r}
write.csv(ca_consumers_n_gas, "data/gold/ca_consumers_n_gas.csv", row.names = FALSE)
```

```{r}
plot_ly(data = ca_consumers_n_gas, x = ~ index, y = ~ y, type = "scatter", mode = "lines") |>
...
```{r}
plot_ly(data = us_natural_gas, x = ~ date, y = ~ y, type = "scatter", mode = "lines") |>
...
```{r}
write.csv(ca_electricty, "data/gold/ca_electricty.csv", row.names = FALSE)
```
...
```{r}
save(ca_consumers_n_gas, us_natural_gas, ca_electricty, 
file = paste(here::here(),"data/silver/energy.RData", sep = "/"))
```