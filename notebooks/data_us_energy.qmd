...
us_natural_gas <- ts_2 |>
    dplyr::mutate(
        index = lubridate::ym(period),
        index2 = yearmonth(index)
    ) |>
    dplyr::select(index, index2, y = value, series_id = duoarea) |>
    dplyr::arrange(index) |>
    as_tsibble(index = index2)
...
write.csv(ca_electricty |> dplyr::select(index, y), file_path, row.names = FALSE)
```
